...
const MAX_BASE64_SIZE = 7 * 1024 * 1024; // 7MB untuk base64 (lebih besar karena encoding)

export const SimpanTwibbon = async (prevState: StateProps | undefined, formData: FormData) => {
  ...
  try {
    ...
    // Validasi ukuran base64 string
    if (image.length > MAX_BASE64_SIZE) {
      return {
        status: 400,
        message: `Gambar terlalu besar! Ukuran maksimal adalah ${MAX_FILE_SIZE / (1024 * 1024)} MB`,
      };
    }
    if (thumbnail.length > MAX_BASE64_SIZE) {
      return {
        status: 400,
        message: `Thumbnail terlalu besar! Ukuran maksimal adalah ${MAX_FILE_SIZE / (1024 * 1024)} MB`,
      };
    }
    ...
    if (buffer.length > 0 || thumbnailBuffer.length > 0) {
      if (buffer.length > MAX_FILE_SIZE || thumbnailBuffer.length > MAX_FILE_SIZE) {
        console.log("Kelebihan file size nya! MAX 5mb");
        return {
          status: 0,
          fields: "",
          message: `File terlalu besar! Ukuran maksimal adalah ${MAX_FILE_SIZE / (1024 * 1024)} Mb`,
        };
      }
      ...
      const matches2 = thumbnail.match(/^data:(image\/(jpeg|jpg|png|webp));base64,(.+)$/);
      if (!matches || !matches2) {
        return {
          status: 0,
          fields: "",
      ...
      const imgName = `${Date.now()}-${Math.random().toString(36).substr(2, 9)}.${ext}`;
      const imgName2 = `${Date.now()}-${Math.random().toString(36).substr(2, 9)}.${ext2}`;
